...
class Song():
    ...
    class Sinfo:
        ...
        def format_output(self, playtype):
            ...
            embed = discord.Embed(title=playtype, description="[{}]({})".format(self.title, self.webpage_url), color=cfg.EMBED_COLOR)

            if self.thumbnail is not None:
                embed.set_thumbnail(url=self.thumbnail)

            embed.add_field(name=cfg.SONGINFO_UPLOADER,
                            value=self.uploader, inline=False)

            if self.duration is not None:
                embed.add_field(name=cfg.SONGINFO_DURATION,
                                value="{}".format(str(datetime.timedelta(seconds=self.duration))), inline=False)
            else:
                embed.add_field(name=cfg.SONGINFO_DURATION,
                                value=cfg.SONGINFO_UNKNOWN_DURATION , inline=False)

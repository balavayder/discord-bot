...
class AudioController(object):
    ...
    def track_history(self):
        history_string = cfg.INFO_HISTORY_TITLE
    ...
    async def play_song(self, song):
        ...
        if song.info.title == None:
            ...
            try:
                downloader = yt_dlp.YoutubeDL(
                    {'format': 'bestaudio', 'title': True, "cookiefile": cfg.COOKIE_PATH})
                r = downloader.extract_info(
                    song.info.webpage_url, download=False)
            except:
                asyncio.wait(1)
                downloader = yt_dlp.YoutubeDL(
                    {'title': True, "cookiefile": cfg.COOKIE_PATH})
                r = downloader.extract_info(
        ...
        for song in list(self.playlist.playque)[:cfg.MAX_SONG_PRELOAD]:
    ...
    async def process_song(self, track):
        ...
        try:
            ...
            try:
                ...
                if "ERROR: Brak wsparcia dla +18" in str(e):
                    return None
        except:
            downloader = yt_dlp.YoutubeDL(
                {'title': True, "cookiefile": cfg.COOKIE_PATH})
    ...
    async def preload(self, song):
        ...
        executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=cfg.MAX_SONG_PRELOAD)
    ...
    async def uconnect(self, ctx):
        ...
        if self.guild.voice_client == None:
            ...
            await ctx.send(cfg.ALREADY_CONNECTED_MESSAGE)

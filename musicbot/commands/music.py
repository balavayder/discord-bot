...
class Music(commands.Cog):
    ...
    async def _play_song(self, ctx, *, track: str):
        ...
        if audiocontroller.playlist.loop == True:
            await ctx.send("Kolejkowanie piosenki włączone! Użyj {}loop aby wyłączyć".format(config.BOT_PREFIX))
            return
    ...
    async def _loop(self, ctx):
        ...
        if audiocontroller.playlist.loop == False:
            ...
            await ctx.send("Kolejkowanie włączone :arrows_counterclockwise:")
        else:
            audiocontroller.playlist.loop = False
            await ctx.send("Kolejkowanie wyłączone :x:")
    ...
    async def _shuffle(self, ctx):
        ...
        if current_guild.voice_client is None or not current_guild.voice_client.is_playing():
            await ctx.send("Kolejka jest pusta :x:")
            return

        audiocontroller.playlist.shuffle()
        await ctx.send("Wymieszano kolejkę :twisted_rightwards_arrows:")
    ...
    async def _pause(self, ctx):
        ...
        await ctx.send("Zapauzowano :pause_button:")
    ...
    async def _stop(self, ctx):
        ...
        await ctx.send("Zatrzymano wszystkie sesje :octagonal_sign:")
    ...
    async def _move(self, ctx, *args):
        if len(args) != 2:
            ctx.send("Zła ilość argumentów")
            return

        try:
            oldindex, newindex = map(int, args)
        except ValueError:
            ctx.send("Zły argument")
            return
        ...
        try:
            ...
            await ctx.send("Zła pozycja")
            return
        await ctx.send("Przeniesiono")
    ...
    async def _skip(self, ctx):
        ...
        await ctx.send("Pominięto aktualną piosenkę :fast_forward:")
    ...
    async def _clear(self, ctx):
        ...
        current_guild.voice_client.stop()
        audiocontroller.playlist.loop = False
        await ctx.send("Wyczyszczono kolejkę :no_entry_sign:")
    ...
    async def _prev(self, ctx):
        ...
        await ctx.send("Gram poprzdnią piosenkę :track_previous:")
    ...
    async def _resume(self, ctx):
        ...
        await ctx.send("Wznowiono granie :arrow_forward:")
    ...
    async def _volume(self, ctx, *args):
        ...
        if len(args) == 0:
            await ctx.send("Aktualna głośność: {}% :speaker:".format(utils.guild_to_audiocontroller[ctx.guild]._volume))
            return
        ...
        try:
            ...
            if utils.guild_to_audiocontroller[current_guild]._volume >= volume:
                await ctx.send('Ustawiono głośność na {}% :sound:'.format(str(volume)))
            else:
                await ctx.send('Ustawiono głośność na {}% :loud_sound:'.format(str(volume)))
            utils.guild_to_audiocontroller[current_guild].volume = volume
        except:
            await ctx.send("Error: Głośność musi wynośić 1-100")

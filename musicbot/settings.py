...
class Settings():
    ...
    async def format(self):
        embed = discord.Embed(
            title="Settings", description=self.guild.name, color=cfg.EMBED_COLOR)

        embed.set_thumbnail(url=self.guild.icon_url)
        embed.set_footer(
            text="Użycie: {}set setting_name value".format(cfg.BOT_PREFIX))
    ...
    async def default_nickname(self, setting, value, ctx):
        ...
        if len(value) > 32:
            await ctx.send(
                "`Error: Osiągnięto limit znaków`\nUżycie: {}set {} nickname\nInne opcje: unset".format(cfg.BOT_PREFIX,
                                                                                                        setting))
            return False
        else:
            ...
            try:
                ...
                await ctx.send("`Error: Nie można zmienić nazwy, prawdopodobnie brak uprawnień")
    ...
    async def start_voice_channel(self, setting, value, ctx):
        ...
        if found == False:
            await ctx.send(
                "`Error: Nie znaleziono VC o takiej nazwie`\nUżycie: {}set {} VCname\nInne opcje: unset".format(
                    cfg.BOT_PREFIX, setting))
            return False
    ...
    async def user_must_be_in_vc(self, setting, value, ctx):
        if value.lower() == "true":
            ...
            await ctx.send(
                "`Error: Wartość musi być TRUE/FALSE`\nUżycie: {}set {} True/False".format(cfg.BOT_PREFIX, setting))
            return False
    ...
    async def button_emote(self, setting, value, ctx):
        ...
        if emoji is None:
            await ctx.send(
                "`Error: Nie znaleziono takiej emotki na serwerze`\nUżycie: {}set {} emotename\nInne opcje: unset".format(
                    cfg.BOT_PREFIX, setting))
            return False
        else:
            self.config[setting] = value
    ...
    async def default_volume(self, setting, value, ctx):
        try:
            ...
            await ctx.send("`Error: Wartość to musi być int`\nUżycie: {}set {} 0-100".format(cfg.BOT_PREFIX, setting))
            return False
    ...
    async def vc_timeout(self, setting, value, ctx):
        ...
        if cfg.ALLOW_VC_TIMEOUT_EDIT == False:
            await ctx.send("`Error: Ta wartość nie może być zmieniona".format(cfg.BOT_PREFIX, setting))
        ...
        if value.lower() == "true":
            ...
            await ctx.send(
                "`Error: Ta wartość musi być True/False`\nUżycie: {}set {} True/False".format(cfg.BOT_PREFIX, setting))
            return False
